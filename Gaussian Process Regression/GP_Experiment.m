...
% %% inspect data
% tiledlayout(1,3)
% plot(D_t.r_x,D_t.M_x)
% nexttile(2)
% plot(D_t.r_y,D_t.M_y)
% nexttile(3)
% plot(D_t.r_z,D_t.M_z)


% %% Use log of M to hihlihgt small changes. 
% D_t.M_x = log(D_t.M_x); 
...
figure(1)
scatter3(D_t.r_x(1:50),D_t.r_y(1:50),D_t.M_z(1:50),50,'r');
hold on
scatter3(D_t.r_x(1:50),D_t.r_y(1:50),ypred_Mz,70,'b','.'); 
xlabel('X_{ECI}')
ylabel('Y_{ECI}')
zlabel('M_z')
...
plot(D_t.r_z,D_t.M_z,'b.');
hold on;
plot(D_t.r_z(1:50),ypred_Mz,'r','LineWidth',1.5);
xlabel('r_z');
ylabel('M_z');
